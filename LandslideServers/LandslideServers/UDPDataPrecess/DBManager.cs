...
namespace DBoperation
{
    class DBManager
    {
        ...
        public DBManager()
        {
            ...
            string connStr = "server= " + read.ip + ";user=" + read.user + ";password=" + read.password + ";database=" + read.database + ";charset=" + read.charset;
            if (conn == null)
            {
        ...
        public bool Insert(string sqlstr)
        {
            try
            {
                ...
                using (MySqlCommand command = new MySqlCommand(sqlstr, conn))//using 释放了所有资源可能会导致一些问题
                {
                    command.ExecuteNonQuery();
                    return true;
                }
