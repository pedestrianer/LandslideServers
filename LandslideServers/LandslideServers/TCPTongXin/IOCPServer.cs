            ...
            if (e.SocketError == SocketError.Success)//if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)
            {
                ...
                if (e.BytesTransferred > 0)
                {
                    ...
                    if (s.Available == 0)
                    {
                        ...
                        //byte[] senData = Encoding.ASCII.GetBytes("hello");
                       
                        //s.SendTo(senData, s.RemoteEndPoint);
                        Dictionary<Socket, byte[]> sendMeg = _oderAnalyze.TOrderAnalyze(e.BytesTransferred, data, s);
                        if (sendMeg != null)
                        {
                            foreach (var key in sendMeg.Keys)
                            {
                                key.SendTo(sendMeg[key], key.RemoteEndPoint);
                            }
                        }
                    }
